...
def chunked_sigma(I: np.ndarray, A: np.ndarray, X: np.ndarray, xp: ModuleType,
                  c: np.ndarray | None = None, chunk: int = 1_000_000) -> float:
    """RMS of the pixel-step residual ``I - c - A @ X``, over pixel chunks.

    The residual is accumulated chunk by chunk in float64, so no second
    ``(N, P)`` array is held.
...
def aia_diagnostics(I: np.ndarray, delta_fit: np.ndarray, g: np.ndarray, a: np.ndarray,
                    ...
                    converged: bool, c: np.ndarray | None = None) -> AIAParam:
    ...
    sigma = chunked_sigma(I, A, xp.vstack([a, u, v]), xp, c=c)
