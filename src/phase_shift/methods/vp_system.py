...
"""Normal system of the VP-AIA first-order fit, ``docs/vp_aia.md`` Eqs. (13)-(15).

Eq. (11) holds at every pixel and contains only the ``2N + NJ`` unknowns
``P_n^(1)``, ``Q_n^(1)`` and ``alpha_nj``, so the least-squares fit reduces to
one dense system whose size does not grow with the pixel count. Eq. (C2) turns
...
class VPSolution:
    ...
    kappa_vp : float
        ...
        independent of the pixel count. Large values mean Eq. (11) has a
...
def max_modes(N: int, K: int) -> int:
    """Largest identifiable number of spatial modes, ``docs/vp_aia.md`` Eq. (7).
...
def _pixel_sums(stack: np.ndarray, a: np.ndarray, u: np.ndarray, v: np.ndarray,
    ...
    """Accumulate every sum of ``docs/vp_aia.md`` Eqs. (C1)-(C2) in one pass.

    The residual of Eq. (8) is formed one pixel block at a time, so no
...
def fit_frame_and_coeffs(stack: np.ndarray, a: np.ndarray, u: np.ndarray, v: np.ndarray,
    ...
    Solves ``docs/vp_aia.md`` Eq. (15), ``beta = (M + C'C)^-1 b``, the
    least-squares fit of Eq. (11) over all pixels. By the Frisch-Waugh-Lovell
    theorem this gives the same unknowns as a joint fit of Eq. (9) that
    includes the pixel corrections; those follow from Eq. (16).
    ...
    a, u, v : np.ndarray, shape (P,)
        ...
        §"VP-AIA algorithm" step 1 prescribes. Eq. (12) assumes ``Pi`` leaves
    ...
    -----
    UserWarning
        If ``M + C'C`` is close to singular, so Eq. (11) has an undetermined
        direction beyond those Eq. (14) fixes.
    """
    if stack.ndim != 2:
    ...
    # Solve in the mode convention of Eq. (20), <H_j H_k> = delta_jk, rather
    ...
    # Eq. (14): the 6 + J directions Pi leaves undetermined, as extra rows.
    ...
    # Eq. (14)'s rows only select among solutions of equal loss, so their
